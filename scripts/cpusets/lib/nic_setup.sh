...
# Default systemd .link file location for EtherCAT PHY pinning. The
# 00- prefix ensures lexical priority over netplan-generated .link
# files (typically /run/systemd/network/10-netplan-*.link) so udevd
# matches our PHY settings first on every device-add event.
#
# Why this matters: EtherCAT slaves advertise only 100baseT/Half and
# don't auto-negotiate. The dispatcher-based persistence path in
# networkd-dispatcher/routable.d only fires when systemd-networkd
# manages the iface — on hosts where NetworkManager owns L3 (e.g.
# hw-thor01) it never runs. A systemd .link file is the only stack-
# layer that re-applies PHY settings on every link-up regardless of
# who owns L3.
: "${NIC_LINK_FILE_DIR:=/etc/systemd/network}"
: "${NIC_LINK_FILE_PREFIX:=00-}"
: "${NIC_LINK_FILE_SUFFIX:=-phy.link}"
...
# ---- nic_link_file_path <iface> --------------------------------------------
#
# Echo the .link file path for <iface>: e.g. ecat1 ->
# /etc/systemd/network/00-ecat1-phy.link. Pure helper, no side effects.
nic_link_file_path() {
    local iface="${1:-}"
    [[ -n "$iface" ]] || return 1
    printf '%s/%s%s%s\n' \
        "$NIC_LINK_FILE_DIR" "$NIC_LINK_FILE_PREFIX" "$iface" "$NIC_LINK_FILE_SUFFIX"
}

# ---- nic_link_file_present <iface> <mac> -----------------------------------
#
# Returns 0 if the .link file for <iface> exists, binds the desired MAC
# (via PermanentMACAddress=, case-insensitive), and pins PHY settings
# (Autonegotiation=no, BitsPerSecond=100M, Duplex=full). Returns 1 if
# the file is missing or any required setting is absent. No side
# effects.
#
# Pair with nic_write_link_file: predicate is the "skip if already
# correct" guard, matching the idempotency invariant of the udev rule
# helpers.
nic_link_file_present() {
    local iface="${1:-}"
    local want_mac="${2:-}"
    [[ -n "$iface" && -n "$want_mac" ]] || return 1

    local file
    file=$(nic_link_file_path "$iface") || return 1
    [[ -f "$file" ]] || return 1

    local want_mac_lc="${want_mac,,}"
    local content_lc
    content_lc=$(tr '[:upper:]' '[:lower:]' < "$file" 2>/dev/null) || return 1

    [[ "$content_lc" == *"permanentmacaddress=$want_mac_lc"* ]] || return 1
    [[ "$content_lc" == *"autonegotiation=no"* ]] || return 1
    [[ "$content_lc" == *"bitspersecond=100m"* ]] || return 1
    [[ "$content_lc" == *"duplex=full"* ]] || return 1
    return 0
}

# ---- nic_write_link_file <iface> <mac> -------------------------------------
#
# Idempotently install a systemd .link file at nic_link_file_path that
# pins PHY settings (BitsPerSecond=100M, Duplex=full, Autonegotiation=no)
# for the NIC identified by <mac>. The [Match] block is keyed on
# PermanentMACAddress so the file follows the hardware regardless of
# kernel iface rename.
#
# Returns 0 on success, 1 on failure. To apply without reboot:
#     udevadm trigger --action=add /sys/class/net/<iface>
# Or `udevadm test-builtin net_setup_link /sys/class/net/<iface>` to
# preview without applying.
nic_write_link_file() {
    local iface="${1:-}"
    local mac="${2:-}"
    if [[ -z "$iface" || -z "$mac" ]]; then
        echo "nic_write_link_file: requires <iface> <mac>" >&2
        return 1
    fi
    if ! nic_validate_iface_name "$iface"; then
        echo "nic_write_link_file: invalid iface name: $iface" >&2
        return 1
    fi

    local file
    file=$(nic_link_file_path "$iface")
    local dir
    dir=$(dirname "$file")
    if [[ ! -d "$dir" ]]; then
        if ! $NIC_SUDO mkdir -p "$dir"; then
            echo "nic_write_link_file: failed to create $dir" >&2
            return 1
        fi
    fi
    ...
    local tmp
    tmp=$(mktemp 2>/dev/null) || {
        echo "nic_write_link_file: mktemp failed" >&2
        return 1
    }

    cat > "$tmp" <<EOF
# EtherCAT PHY pinning for $iface (MAC $mac_lc).
# Auto-generated by nic_setup.sh; do not edit by hand.
#
# Applied by systemd-udevd on every device-add event regardless of
# whether systemd-networkd, NetworkManager, or nothing manages L3.
# EtherCAT slaves advertise only 100baseT/Half and don't auto-negotiate,
# so autoneg MUST stay off for the bus to come up.
[Match]
PermanentMACAddress=$mac_lc
...
EOF
    ...
    if ! $NIC_SUDO install -m 0644 "$tmp" "$file"; then
        rm -f "$tmp"
        echo "nic_write_link_file: failed to install $file" >&2
        return 1
    fi
    rm -f "$tmp"
    return 0
