...
#   --policy-diagnostics-config
#                        render+apply the policy-diagnostics-config ConfigMap
#                        (DIAG_* knobs incl. per-joint motor-gain overrides;
#                        sourced from host-config phantomPolicyDiagnostics).
...
SKIP_SONIC_CONFIG=0
SKIP_PSI_CONFIG=0
SKIP_POLICY_DIAGNOSTICS_CONFIG=0
SKIP_GITOPS=0
SKIP_ARGOCD_ADMIN=0
...
WORKLOAD_NAMESPACES=(argocd argus dma-video gaia nimbus phantom positronic psi)
...
PULL_SECRET_NAMESPACES=(argus dma-video gaia nimbus phantom positronic psi)
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --policy-diagnostics-config)
                         SELECTED_PHASES+=(policy-diagnostics-config); shift ;;
...
if [ "${#SELECTED_PHASES[@]}" -gt 0 ]; then
  ...
  SKIP_SONIC_CONFIG=1
  SKIP_PSI_CONFIG=1
  SKIP_POLICY_DIAGNOSTICS_CONFIG=1
  ...
  for _p in "${SELECTED_PHASES[@]}"; do
    case "$_p" in
      ...
      policy-diagnostics-config) SKIP_POLICY_DIAGNOSTICS_CONFIG=0 ;;
...
# ---- phase 8c: policy-diagnostics-config (DIAG_* ConfigMap) -------

# Per-host policy-diagnostics options file. Holds a ConfigMap manifest the
# policy-diagnostics DaemonSet's container reads via envFrom (optional:true).
# Same host-resident, ArgoCD-unmanaged, --reset-preserved lifecycle as the
# locomotion CM. The DIAG_* values (incl. per-joint motor-gain overrides)
# are sourced from host-config's phantomPolicyDiagnostics block.
POLICY_DIAGNOSTICS_FILE="${POLICY_DIAGNOSTICS_FILE:-/etc/phantomos/policy-diagnostics-config.yaml}"
POLICY_DIAGNOSTICS_NS="positronic"
POLICY_DIAGNOSTICS_CM_NAME="policy-diagnostics-config"

_write_policy_diagnostics_file() {
  # Render a single multi-line KEY=VALUE blob (as produced by the
  # host-config helper's `get-policy-diagnostics-config-kv` subcommand) into
  # one YAML-quoted `KEY: "VALUE"` entry per non-empty line under the
  # ConfigMap's data: section.
  local kv_text="${1?_write_policy_diagnostics_file: kv_text required}"
  mkdir -p "$(dirname "$POLICY_DIAGNOSTICS_FILE")"
  {
    cat <<EOF
# Generated by scripts/bootstrap-robot.sh — do not hand-edit.
# Re-run bootstrap with --policy-diagnostics-config to change these values.
apiVersion: v1
kind: ConfigMap
metadata:
  name: $POLICY_DIAGNOSTICS_CM_NAME
  namespace: $POLICY_DIAGNOSTICS_NS
data:
EOF
    local line key value
    while IFS= read -r line; do
  ...
  chmod 0644 "$POLICY_DIAGNOSTICS_FILE"
}

policy_diagnostics_config() {
  if [ "${SKIP_POLICY_DIAGNOSTICS_CONFIG:-0}" = 1 ]; then
    phase "phase 8c: policy-diagnostics-config  (skipped)"
    return
  fi
  phase "phase 8c: policy-diagnostics-config (policy-diagnostics-config ConfigMap)"
  ...
  local kv_text=""
  if [ -r "$hc" ]; then
    kv_text=$(python3 "$HOST_CONFIG_HELPER" "$hc" get-policy-diagnostics-config-kv 2>/dev/null || true)
  fi
  if [ -z "$kv_text" ]; then
    # Helper failed (no host-config, validation error, etc.) — the
    # DaemonSet's envFrom is optional:true, so a missing/empty CM still
    # lets the launcher fall back to its own DIAG_* defaults. Emit a
    # minimal CM (no data:) so kubectl apply still succeeds.
    kv_text=""
  fi
  ...
  if [ ${#KUBECTL[@]} -eq 0 ]; then
    fail "no kubectl/k0s available — cannot apply policy-diagnostics ConfigMap"
    return
  fi

  if ! "${KUBECTL[@]}" get ns "$POLICY_DIAGNOSTICS_NS" >/dev/null 2>&1; then
    if ! "${KUBECTL[@]}" create ns "$POLICY_DIAGNOSTICS_NS" >/dev/null; then
      fail "could not create ns/$POLICY_DIAGNOSTICS_NS"
      return
    fi
    info "created ns/$POLICY_DIAGNOSTICS_NS"
  fi

  _write_policy_diagnostics_file "$kv_text"
  pass "wrote $POLICY_DIAGNOSTICS_FILE  ($(printf '%s\n' "$kv_text" | grep -c '^DIAG_' || true) DIAG_* keys)"

  if ! "${KUBECTL[@]}" apply -f "$POLICY_DIAGNOSTICS_FILE" >/dev/null; then
    fail "kubectl apply -f $POLICY_DIAGNOSTICS_FILE"
    return
  fi
  ...
  # If the DaemonSet is already running, restart it so the new options take
  # effect. envFrom does NOT auto-roll on CM updates.
  if "${KUBECTL[@]}" -n "$POLICY_DIAGNOSTICS_NS" get ds policy-diagnostics >/dev/null 2>&1; then
    if "${KUBECTL[@]}" -n "$POLICY_DIAGNOSTICS_NS" rollout restart ds/policy-diagnostics >/dev/null; then
      pass "rolled out policy-diagnostics DaemonSet to pick up new options"
    else
      fail "rollout restart ds/policy-diagnostics"
    fi
  else
    info "ds/policy-diagnostics not present yet — gitops phase will create it with the new CM in scope"
  fi
}
...
print_plan() {
  ...
  _step $([ "$SKIP_POLICY_DIAGNOSTICS_CONFIG" = 0 ] && echo 1 || echo 0) "phase  8c policy-diagnostics-config"  "--policy-diagnostics-config not selected"
...
sonic_config       ; guard
psi_config         ; guard
policy_diagnostics_config ; guard
