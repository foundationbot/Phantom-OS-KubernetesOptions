...
#   --positronic-config  merge POSITRONIC_MODE + POSITRONIC_DIAGNOSTIC_*
#                        (and the diagnostic-launcher PHANTOM_CMD) into
#                        the positronic-config ConfigMap, sourced from
#                        host-config positronicControl.{mode,diagnostic}.
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --positronic-config) SELECTED_PHASES+=(positronic-config); shift ;;
...
if [ "${#SELECTED_PHASES[@]}" -gt 0 ]; then
  ...
  for _p in "${SELECTED_PHASES[@]}"; do
    case "$_p" in
      ...
      positronic-config) SKIP_POSITRONIC_CONFIG=0 ;;
...
# ---- phase 6.6: positronic-config (POSITRONIC_MODE ConfigMap) -------

# Per-host positronic_control config. Renders the POSITRONIC_MODE +
# POSITRONIC_DIAGNOSTIC_* env set (and, in diagnostic mode, the
# diagnostic-launcher PHANTOM_CMD) into the positronic-control
# DaemonSet's positronic-config ConfigMap, which the pod reads via
# envFrom. positronic analogue of phase 6.5 (locomotion-config).
#
# IMPORTANT delivery difference from locomotion: phantom-locomotion-
# config is NOT in any kustomization, so locomotion_config can safely
# `kubectl apply` a host-rendered file (ArgoCD never manages it).
# positronic-config IS gitops-managed (manifests/base/positronic/
# kustomization.yaml lists configmap.yaml), so a raw `kubectl apply -f`
# of a host file would drift and get reverted on the next Argo sync.
# We therefore deliver these keys the same drift-safe way PHANTOM_CMD
# already flows (FIR-407/408): as a strategic-merge entry in the core
# Argo Application's spec.source.kustomize.patches on
# ConfigMap/positronic-config, reusing the same target identity as
# _patch_positronic_phantom_cmd so the two patches coexist on one CM.
#
# We still render a host-side file (/etc/phantomos/positronic-config.yaml)
# as a human-readable record / for `--dry-run` inspection, mirroring
# _write_locomotion_file, but the AUTHORITATIVE delivery is the Argo
# patch.
POSITRONIC_FILE="${POSITRONIC_FILE:-/etc/phantomos/positronic-config.yaml}"
POSITRONIC_NS="positronic"
POSITRONIC_CM_NAME="positronic-config"

_write_positronic_file() {
  # Render a human-readable KEY: "VALUE" ConfigMap snapshot of the
  # positronic diagnostic env into POSITRONIC_FILE. Mirrors
  # _write_locomotion_file. Not applied directly (see phase comment) —
  # the Argo patch in positronic_config() is the authoritative delivery.
  local kv_text="${1?_write_positronic_file: kv_text required}"
  mkdir -p "$(dirname "$POSITRONIC_FILE")"
  {
    cat <<EOF
# Generated by scripts/bootstrap-robot.sh — do not hand-edit.
# Human-readable snapshot of the positronic-config keys delivered via
# the core Argo Application's kustomize.patches. Re-run bootstrap with
# --positronic-config to refresh.
apiVersion: v1
kind: ConfigMap
metadata:
  name: $POSITRONIC_CM_NAME
  namespace: $POSITRONIC_NS
data:
EOF
    local line key value
    while IFS= read -r line; do
  ...
  chmod 0644 "$POSITRONIC_FILE"
}

positronic_config() {
  if [ "${SKIP_POSITRONIC_CONFIG:-0}" = 1 ]; then
    phase "phase 6.6: positronic-config  (skipped)"
    return
  fi
  phase "phase 6.6: positronic-config (POSITRONIC_MODE ConfigMap)"
  ...
  local kv_text=""
  if [ -r "$hc" ]; then
    kv_text=$(python3 "$HOST_CONFIG_HELPER" "$hc" get-positronic-config-kv 2>/dev/null || true)
  fi
  if [ -z "$kv_text" ]; then
    # Helper failed (no host-config, validation error, etc.) — fall back
    # to the documented default (production).
    kv_text="POSITRONIC_MODE=production"
  fi

  # Extract a short summary for the log lines.
  local mode_line mode
  mode_line=$(printf '%s\n' "$kv_text" | grep -m1 '^POSITRONIC_MODE=' || true)
  mode="${mode_line#POSITRONIC_MODE=}"

  if [ "$DRY_RUN" = 1 ]; then
    info "DRY-RUN  write $POSITRONIC_FILE  POSITRONIC_MODE=$mode"
    info "DRY-RUN  merge positronic-config keys into core Argo app kustomize.patches"
    if [ "$mode" = "production" ]; then
      info "DRY-RUN  production mode — POSITRONIC_MODE only; PHANTOM_CMD untouched (FIR-407/408 path owns it)"
    fi
    return
  fi

  _write_positronic_file "$kv_text"
  pass "wrote $POSITRONIC_FILE  POSITRONIC_MODE=$mode"

  if [ ${#KUBECTL[@]} -eq 0 ]; then
    info "no kubectl/k0s available — skipping positronic-config Argo patch"
    return
  fi

  local app="phantomos-$ROBOT-core"
  if ! "${KUBECTL[@]}" -n argocd get app "$app" >/dev/null 2>&1; then
    info "Application $app not present — gitops phase will create it; re-run --positronic-config after"
    return
  fi

  # In production mode there is nothing positronic-specific to merge here
  # (POSITRONIC_MODE is informational; PHANTOM_CMD is owned by the
  # launchCommand path). We still SCRUB any prior diagnostic patch so
  # flipping back to production cleanly reverts the CM.
  ...
  # Compute the diagnostic-env data map (everything EXCEPT PHANTOM_CMD,
  # which the FIR-407/408 sub-phase owns) and merge it as a dedicated
  # kustomize patch on ConfigMap/positronic-config. We give the patch a
  # distinct logical identity from the PHANTOM_CMD patch by carrying the
...
# A marker so we can find+replace THIS patch on re-runs without touching
# the PHANTOM_CMD patch (which carries data.PHANTOM_CMD only).
def is_positronic_env_entry(entry):
    if not isinstance(entry, dict):
        return False
    tgt = entry.get("target") or {}
    if not (tgt.get("kind") == TARGET_KIND
            and tgt.get("name") == TARGET_NAME
            and tgt.get("namespace") == TARGET_NS):
        return False
    # Distinguish our env patch from the PHANTOM_CMD patch: ours sets
    # POSITRONIC_MODE in its rendered patch body.
    return "POSITRONIC_MODE" in (entry.get("patch") or "")

filtered = [e for e in current if not is_positronic_env_entry(e)]
...
PY
    ...
    fail "failed to compute positronic-config kustomize.patches merge (PyYAML missing?)"
    return
  fi

  if "${KUBECTL[@]}" -n argocd patch app "$app" --type=merge -p "$merge_patch" >/dev/null; then
    pass "$app  positronic-config env merged (POSITRONIC_MODE=$mode)"
  else
    fail "kubectl patch app $app positronic-config env failed"
    return
  fi
  ...
  # Roll the DaemonSet so the new env takes effect (envFrom is read once
  # at container start). Only when the DS already exists.
  if "${KUBECTL[@]}" -n "$POSITRONIC_NS" get ds positronic-control >/dev/null 2>&1; then
    if "${KUBECTL[@]}" -n "$POSITRONIC_NS" rollout restart ds/positronic-control >/dev/null; then
      pass "rolled out positronic-control DaemonSet to pick up POSITRONIC_MODE=$mode"
    else
      fail "rollout restart ds/positronic-control"
    fi
  else
    info "ds/positronic-control not present yet — gitops phase will create it with the new env in scope"
  fi
}
...
print_plan() {
  ...
  _step $([ "$SKIP_POSITRONIC_CONFIG"    = 0 ] && echo 1 || echo 0) "phase  6.6 positronic-config"           "--positronic-config not selected"
