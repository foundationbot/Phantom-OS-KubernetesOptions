...
#
# Derives two additional keys from the URL's host portion so the vr-web
# Deployment doesn't need to re-encode the AI PC location:
#   CAMERA_SERVER_URL  -> http[s]://<host>:8889  (MediaMTX WHEP)
#   ROSBRIDGE_URL      -> ws[s]://<host>:9090    (rosbridge_server)
# The vr-web pod's nginx substitutes these into __CAMERA_PROXY__ /
# __ROSBRIDGE_PROXY__ at start; operator-ui ignores them and continues
# to read AI_PC_URL only.
_write_pairing_file() {
  local url="${1:?_write_pairing_file: url required}"
  local rest="${url#*://}"
  local host="${rest%%[:/]*}"
  if [ -z "$host" ]; then
    fail "could not extract host from --ai-pc-url '$url'"
    return 1
  fi
...
# Best-effort upgrade for pre-FIR-330 pairing files that still hold the
# legacy AI_PC_URL key (and possibly the derived CAMERA_SERVER_URL /
# ROSBRIDGE_URL). Extracts the host portion and rewrites the file in the
# new single-key shape. Returns 0 on success, 1 when the file is already
# new-format (so the caller falls back to re-applying as-is).
_migrate_legacy_pairing_file() {
  [ -r "$PAIRING_FILE" ] || return 1
  if grep -q '^[[:space:]]*AI_PC_HOST:' "$PAIRING_FILE"; then
    return 1  # already new format
  fi
  local legacy
  legacy=$(awk '
  ...
  [ -n "$legacy" ] || return 1
  info "migrating legacy $PAIRING_FILE (AI_PC_URL -> AI_PC_HOST)"
  _write_pairing_file "$legacy"
}
...
operator_ui_config() {
  ...
  if [ "$DRY_RUN" = 1 ]; then
    if [ -n "$AI_PC_URL" ]; then
      info "DRY-RUN  write $PAIRING_FILE with AI_PC_HOST extracted from $AI_PC_URL"
      info "DRY-RUN  kubectl apply -f $PAIRING_FILE"
    elif [ -r "$PAIRING_FILE" ]; then
      info "DRY-RUN  migrate-if-legacy + re-apply existing $PAIRING_FILE"
    else
  ...
  if [ -n "$AI_PC_URL" ]; then
    ...
    _write_pairing_file "$AI_PC_URL" || return
    pass "wrote $PAIRING_FILE  AI_PC_HOST=$(awk '/^[[:space:]]*AI_PC_HOST:/{print $2}' "$PAIRING_FILE")"
  elif [ ! -r "$PAIRING_FILE" ]; then
    fail "$PAIRING_FILE missing — first bringup needs --ai-pc-url <url>"
    return
  else
    # Auto-upgrade pre-FIR-330 files that still hold AI_PC_URL.
    _migrate_legacy_pairing_file || info "re-applying existing $PAIRING_FILE"
  fi
