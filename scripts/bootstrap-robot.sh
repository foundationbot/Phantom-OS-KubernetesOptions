...
operator_ui_config() {
  ...
  if [ "$DRY_RUN" = 1 ]; then
    if [ -n "$AI_PC_URL" ]; then
      info "DRY-RUN  write $PAIRING_FILE with AI_PC_HOST=<from $AI_PC_URL>, CONTROL_PC_HOST=<_resolve_api_address>"
      info "DRY-RUN  kubectl apply -f $PAIRING_FILE"
    elif [ -r "$PAIRING_FILE" ]; then
      info "DRY-RUN  rewrite $PAIRING_FILE preserving AI_PC_HOST, refreshing CONTROL_PC_HOST"
    else
  ...
  _write_pairing_file "$ai_pc_host" "$control_pc_host" || return
  pass "wrote $PAIRING_FILE  AI_PC_HOST=$ai_pc_host  CONTROL_PC_HOST=$control_pc_host"
