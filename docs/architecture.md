...
| `phantomos-app-core.yaml` | Rendered ArgoCD Application CR for the `core` stack (`phantomos-<robot>-core`). | `bootstrap-robot.sh` phase 9 (`gitops` -> `_gitops_render_app`) | `kubectl apply -f` |
| `phantomos-app-operator.yaml` | Rendered ArgoCD Application CR for the `operator` stack (`phantomos-<robot>-operator`). Absent when `stacks.operator.enabled: false`. | `bootstrap-robot.sh` phase 9 | `kubectl apply -f` |
...
+----------------------------------------------------------------+
                           ...
                           v                                      v
       +----------------------------------+   +----------------------------------+
       | bootstrap-robot.sh phase 6     |   | bootstrap-robot.sh phase 9        |
...
phase 11 indexes which stack each image belongs to (by running kustomize on
...
images:
  ...
  - name: foundationbot/dma-ethercat            # special case: not a stack image;
    newTag: main-latest-aarch64                 # phase 8 reads it directly to
...
Phase 8 closes the gate. The installer Job is bootstrap-managed (lives
...
(image-overrides) silently skips it; phase 8 reads it directly via
...
| `uninstall_ethercat` | stop+disable `dma-ethercat.service`; run `/usr/sbin/dma-ethercat-uninstall` if present. Phase 8 reinstalls fresh from the image |
...
argocd namespace
  ...
  |       kustomize:
  |         images:   [...]   # injected by phase 11
  |         patches:  [...]   # injected by phase 12
...
`stacks.<name>.enabled: false` makes phase 9 delete that Application and its
...
### `spec.source.kustomize.images` — image overrides (phase 11)
...
`NON_STACK_IMAGES` skip-set). Instead, phase 8 (`install-dma-ethercat`)
...
### `spec.source.kustomize.patches` — strategic-merge patches (phase 12)
...
phase 9 started rendering it with sed substitutions and applying the result
...
injected into the live Application via `spec.source.kustomize.images` (phase 11).
