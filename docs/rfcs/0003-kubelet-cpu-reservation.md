...
**Status:** Blocked — implementation hit Jetson topology + kubelet
enforcement issues. See [Implementation findings (2026-05-05)](#implementation-findings-2026-05-05)
for the full chain of experiments and root-cause analysis. The
runtime mitigation in `feat/cpu-isolation-bootstrap` (kubepods in
`MANAGED_SLICES`, re-apply on `isolated invalid`) is what's
shipping; native kubelet integration is parked.
**JIRA:** FIR-282
**Author:** TBD
**Created:** 2026-05-04
